...
static void tide_solid(const double *rsun, const double *rmoon,
                       const double *pos, const double *E, double gmst, int flag,
                       double *dr)
{
    double dr1[3],dr2[3],eu[3],du,dn,sinl,sin2l;
    
    trace(3,"tide_solid: pos=%.3f %.3f flag=%d\n",pos[0]*R2D,pos[1]*R2D,flag);
...
static void tide_pole(gtime_t tut, const double *pos, const double *erpv,
                      double *denu, double *E, double *dr)
{
    double xp_bar,yp_bar,m1,m2,cosl,sinl,drt[3];
    int i;
...
/* solid earth tides */
static void solid_earth_tide(gtime_t tutc, double *erpv, double *pos, double *E, int flag, double *dr) {
    int i;
    double gmst, rs[3], rm[3], drt[3];
#ifdef IERS_MODEL
    double ep[6];
#endif
...
#else
    tide_solid(rs,rm,pos,E,gmst,flag,drt);
#endif
    for (i=0;i<3;i++) dr[i]+=drt[i];
}

/* ocean tide loading */
static void ocean_tide_loading(gtime_t tut, const double *odisp, double *E, double *denu, double *dr) {
    int i;
    double drt[3];
    ...
    matmul("TN",3,1,3,1.0,E,denu,0.0,drt);
    for (i=0;i<3;i++) dr[i]+=drt[i];
    trace(5,"ocean tide loading erp, %.1f, %.6f, %.6f, %.6f\n",time2gpst(tut,NULL),drt[0],drt[1],drt[2]);
}


/* ocean tide loading */
static void ocean_tide_clasgrid(gtime_t tut, olod_t *oload, int *index, int netnum, double *E, double *Emat, double *Gmat, int nn, double *weight, double *dr) {
    int i,j;
    double drt[3],val[4],val_[4],denu_grid[4][3],denu[3]={0};

    for (i=0; i<nn; i++) {
        tide_oload(tut,oload->odisp[index[i]],denu_grid[i]);
        trace(5,"oload->odisp[index[i]]: %d, %d, %.3f\n", i, index[i], oload->odisp[index[i]]);
    }
    if (nn==4&&Gmat&&Emat) {
        for(i=0;i<3;i++) {
            for (j=0;j<nn;j++) {
        ...
        trace(5,"tide_oload: Bilinear denu[%.1f]=%.3f %.3f %.3f\n",time2gpst(tut,NULL),denu[0],denu[1],denu[2]);
    } else {
        denu[0]=denu[1]=denu[2]=0.0;
        for (i=0;i<nn;i++) {
            for(j=0;j<3;j++) {
                denu[j]+=denu_grid[i][j]*weight[i];
            }
        }
    }
    matmul("TN",3,1,3,1.0,E,denu,0.0,drt);
    for (i=0;i<3;i++) dr[i]+=drt[i];
    trace(5,"ocean tide loading(clasgrid), %.1f, %.6f, %.6f, %.6f\n",time2gpst(tut,NULL),drt[0],drt[1],drt[2]);
}
/* tidal displacement ----------------------------------------------------------
* displacements by earth tides
* args   : gtime_t tutc     I   time in utc
*          double *rr       I   site position (ecef) (m)
*          int    tidecorr  I   earth tide correction options (or of the followings)
*                                 0: off
*                                 1: solid 
*                                 2: solid+otl-station+pole 
*                                 3: solid+otl-clasgrid+pole 
...
*                                   8:Mf,9:Mm,10:Ssa)
*          olod_t *oload    I   ocean loading parameters every clasgrid (NULL: not used)
*          int     netnum   I   selected network number 
*          int     nn       I   selected grid number
*          int    *index    I   selected grid index 
*          double *Gmat     I   G matrix
*          double *Emat     I   E matrix
...
*-----------------------------------------------------------------------------*/
extern void tidedisp(gtime_t tutc, const double *rr, int tidecorr, const erp_t *erp,
                     const double *odisp, olod_t *oload, int netnum, int nn, 
                     int *index, double *Gmat, double *Emat, double *weight, double *dr)
{
    gtime_t tut;
    double pos[2],E[9],denu[3]={0},erpv[5]={0};
#ifdef IERS_MODEL
    double ep[6],fhr;
    int year,mon,day;
#endif
    ...
    switch (tidecorr) {
        case 1: 
            /* solid */
            solid_earth_tide(tutc,erpv,pos,E,1,dr);
            break;
        case 2: 
            /* solid+otl-station+pole */
            solid_earth_tide(tutc,erpv,pos,E,1,dr);
            ocean_tide_loading(tut,odisp,E,denu,dr);
            tide_pole(tut,pos,erpv,denu,E,dr);
            break;
        case 3:
            /* solid+otl-clasgrid+pole */
            solid_earth_tide(tutc,erpv,pos,E,1,dr);
            tide_pole(tut,pos,erpv,denu,E,dr);
            ocean_tide_clasgrid(tut,oload,index,netnum,E,Emat,Gmat,nn,weight,dr);
            break;
        case 0: 
            return; 
    }
                   ...
                   double *H, double *R, double *azel)
{
    ...
    int i,j,k,sat,sys,nv=0,nx=rtk->nx,brk;
    ...
    if (opt->tidecorr) {
        tidedisp(gpst2utc(obs[0].time),rr,opt->tidecorr,&nav->erp,opt->odisp[0],
                 NULL,0,0,NULL,NULL,NULL,NULL,disp);
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        for (j=0;j<2;j++) { /* for phase and code */
            ...
            /* test innovation */
#if 0
            if (opt->maxinno[0]>0.0&&fabs(v[nv])>opt->maxinno[0]) {
#else
            if (opt->maxinno[0]>0.0&&fabs(v[nv])>opt->maxinno[0]&&sys!=SYS_GLO) {
#endif
