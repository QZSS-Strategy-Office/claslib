    ...
    if (!revs) { /* input forward data */
        ...
        while (isbs<sbss.n) {
            ...
            if (getbitu(sbss.msgs[isbs].msg,8,6)!=9) { /* except for geo nav */
                sbsupdatecorr(sbss.msgs+isbs,&rtcm.nav);
            }
        ...
        while (ilex<lexs.n) {
            if (lexupdatecorr(lexs.msgs+ilex,&rtcm.nav,&time)) {
                if (timediff(time,obs[0].time)>-1.0-DTTOL) break;
            }
            ilex++;
        }
...
/* process positioning -------------------------------------------------------*/
static void procpos(FILE *fp, prcopt_t *popt, const solopt_t *sopt,
                    int mode)
{
    gtime_t time={0};
    sol_t sol={{0}};
    ...
    if (popt->mode>=PMODE_PPP_RTK) {
        rtkinitppprtk(&rtk,popt);
    } else {
        rtkinit(&rtk,popt);
    }
...
static void combres(FILE *fp, const prcopt_t *popt, const solopt_t *sopt)
{
    ...
    for (i=0,j=isolb-1;i<isolf&&j>=0;i++,j--) {
        ...
        if ((tt=timediff(solf[i].time,solb[j].time))<-DTTOL) {
            ...
            if ((popt->mode==PMODE_KINEMA||popt->mode==PMODE_VRS_RTK||popt->mode==PMODE_MOVEB)&&
                sols.stat==SOLQ_FIX) {
                  ...
                  const prcopt_t *opt)
{
    ...
    if (opt_.sateph!=EPHOPT_BRDC) opt_.sateph=EPHOPT_BRDC;
    
    for (i=0;i<MAXSAT;i++) rtcm.nav.ssr[i].iode=-1;
    ...
    for (iobs=0;(m=nextobsf(obs,&iobs,rcv))>0;iobs+=m) {
        ...
        if (!pntpos(data,j,nav,&opt_,&sol,NULL,NULL,msg)) continue;
                   ...
                   const filopt_t *fopt, nav_t *nav, pcvs_t *pcvs, pcvs_t *pcvr)
{
    ...
    /* read isb parameters */
    if (*fopt->isb) readisb(fopt->isb,nav);
    /* read 1/4 cycle shift table */
    if (*fopt->ifpcs) {
        if ((readL2C(fopt->ifpcs,nav))!=0) {
...
/* read ocean tide loading parameters(clasgrid) ------------------------------*/
extern int readotlgrid(const char *file, nav_t *nav)
{
    get_oload_ptr(nav);
    return readblqgrid(file,nav);
}
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    /* read obs and nav data */
    if (!readobsnav(ts,te,ti,infile,index,n,&popt_,&obss,&rtcm.nav,stas)) return 0;
    
    /* set antenna paramters */
    if (popt_.mode!=PMODE_SINGLE) {
        setpcv(obss.n>0?obss.data[0].time:timeget(),&popt_,&rtcm.nav,&pcvss,&pcvsr,
               stas);
    }
    /* ISB */
    ...
    if (popt_.mode>PMODE_SINGLE&&fopt->blq) {
        ...
        } else if (popt_.tidecorr == 3) {
            /* clasgrid */
            if(!readotlgrid(fopt->blq,&rtcm.nav)){
                freeobsnav(&obss,&rtcm.nav);
                return 0;
            }
        }
    }
    ...
    if (popt_.mode==PMODE_FIXED) {
        if (!antpos(&popt_,1,&obss,&rtcm.nav,stas,fopt->stapos)) {
            freeobsnav(&obss,&rtcm.nav);
            return 0;
        }
    }
    else if (PMODE_DGPS<=popt_.mode&&popt_.mode<=PMODE_STATIC) {
        if (!antpos(&popt_,2,&obss,&rtcm.nav,stas,fopt->stapos)) {
            freeobsnav(&obss,&rtcm.nav);
            return 0;
        }
    }
    ...
    if (popt_.mode>=PMODE_PPP_RTK&&flag) {
        ...
        fprintf(fp_osr, "msg,tow,sys,prn,pbias1,pbias2,pbias5,cbias1,cbias2,cbias5,trop,iono,antr1,antr2,antr5,relatv,wup1,wup2,wup5,compI1,compI2,compI5,compN,CPC1,CPC2,CPC5,PRC1,PRC2,PRC5,orb,clk,lat,lon,alt\n");
    }
                     ...
                     const char *rov, const char *base)
{
    ...
    if (i<n) { /* include base station keywords */
        if (!(base_=(char *)malloc(strlen(base)+1))) {
            freepreceph(&rtcm.nav,&sbss,&lexs);
            return 0;
        }
        strcpy(base_,base);
                   ...
                   const char *rov, const char *base)
{
    ...
    if (!openses(popt,sopt,fopt,&rtcm.nav,&pcvss,&pcvsr)) return -1;
    
    if (ts.time!=0&&te.time!=0&&tu>=0.0) {
        if (timediff(te,ts)<0.0) {
            showmsg("error : no period");
            closeses(&rtcm.nav,&pcvss,&pcvsr);
            return 0;
        }
