...
#define PMODE_PPP_RTK 9                 /* positioning mode: PPP-RTK */
#define PMODE_SSR2OSR 10                /* conversion  mode: SSR2OSR */
#define PMODE_SSR2OSR_FIXED 11          /* conversion  mode: SSR2OSR-fixed */
#define PMODE_VRS_RTK 12                /* positioning mode: relative positioning using clas*/
...
#define COMPOPT_OFF 0                   /* compensatedisp option: off */
#define COMPOPT_SSR 1                   /* compensatedisp option: ssr based */
#define COMPOPT_MEAS 2                  /* compensatedisp option: measurement based  */
...
#define P2_3        0.1250              /* 2^-3 */
#define P2_4        0.06250             /* 2^-4 */
...
#define SSR_CH_NUM    2         /* Input Channels of ssr */
...
typedef struct {        /* observation data record */
    ...
    unsigned short iod; /* iod for correction */
    double L[NFREQ+NEXOBS]; /* observation data carrier-phase (cycle) */
    double P[NFREQ+NEXOBS]; /* observation data pseudorange (m) */
    float  D[NFREQ+NEXOBS]; /* observation data doppler frequency (Hz) */
    double phasecorr;   /* phase correction */
	int    facility;    /* L6 facility(clas 2ch VRS) */
    int osr_i;
} obsd_t;
...
typedef struct {        /* zwd data type */
    ...
    float rms;          /* rms value (m) */
    float quality;      /* tropsphere quality indicator */
} zwdd_t;
...
#define MAX_DEG_VTEC_SSR 8
#define MAX_ORDER_VTEC_SSR 8
...
typedef struct {   /* ocean loading */
    int    gridnum;                     /* number of grid */
    double pos[RTCM_SSR_MAX_GP][2];     /* grid position [deg] */
    double odisp[RTCM_SSR_MAX_GP][6*11];  /* ocean tide loading parameters */
} olod_t;
...
typedef struct {        /* SSR correction type */
    ...
    double pbiasn[CSSR_MAX_NETWORK][CSSR_MAX_SIG]; /* network phase biases (m) */
    float ppbiasl0[MAX_SIGNAL_NUMB];        /* phase biase (m) */
    float l0bias;               /* l0bias  */
    float ppbias[MAX_SIGNAL_NUMB];        /* previous phase biase (m) */
    unsigned int discontinuity[MAXCODE]; /* discontinuity indicator */
    double yaw;         /* satellite yaw angle [rad] */
    double yawr;        /* satellite yaw rate angle [rad/s] */
    unsigned char update; /* update flag (0:no update,1:update) */

    unsigned char update_ura; /* update flag (0:no update,1:update) */
    ...
    unsigned int lng;   /* grid data length */
    int nsig;           /* number of signals used for ssrg cssr */
    int ambfix;         /* reference ambiguity fix flag for ssrg cssr */
    int smode[MAXCODE]; /* signnal mode for ssrg ssrig ssrcg*/
    float psig[MAXCODE]; /* phase stdev (MADOCA extensin) (m) */
    char hold_oc;
    int hold_oc_tow;
    double dlt_dclk;
} ssr_t;
...
typedef struct {
    ...
    int degree;
    int nc, ns;
    float c[153]; /* cos term of VTEC spherical harmonics [TECU] */
    float s[136]; /* sin term of VTEC spherical harmonics [TECU] */
    unsigned char update; /* update flag (0:no update,1:update) */
} ssr_vtec_t;


typedef struct {
    int model;      /* STEC model : 0 : standard */
    float quality[MAX_SAT_STEC_SSR];    /* STEC quality [TECu] */
    int gp_origin;
    double gp0[3];  /* ground point origin [deg/deg/km] */
    int poly_type;      /* polynomial type */
    int nsat;
    int sat[MAX_SAT_STEC_SSR];
    unsigned char flag[MAX_SAT_STEC_SSR];   /* STEC satellite specific flag 0: standard */
    double a[MAX_SAT_STEC_SSR][MAX_COEFS_STEC_SSR]; /* STEC coefficient */
    unsigned char update; /* update flag (0:no update,1:update) */
    int network;
} ssr_stec_t;

typedef struct {        /* RTCM SSR  ground point data d*/
    int type;
    double pos[3];
    int network;
    int update;
...
typedef struct {        /* RTCM SSR gridded troposphere/ionosphere correction type */
    ...
    double udi;         /* SSR update interval (s) */
    int iod;            /* iod ssr */
    int ngp;            /* number of ground points (6) */
    float quality;      /* troposphere quality */
    ssr_gp_t gp[RTCM_SSR_MAX_GP];   /* ground point data */
    float trop_total[RTCM_SSR_MAX_GP];  /* troposphere total vertical delay [m] */
    float trop_wet[RTCM_SSR_MAX_GP];    /* troposphere wet vertical delay [m] */
    int nsv[RTCM_SSR_MAX_GP];
    int sat[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT];
    float stec0[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT];  /* STEC0 [TECu] */
    float stec[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT];   /* STEC [TECu] */
    int updatesat[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT];
    float resp[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT][RTCM_SSR_GP_MAX_SIG];  /* residual of phase [m] */
    float resc[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT][RTCM_SSR_GP_MAX_SIG];  /* residual of code [m] */
    unsigned char nsig[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT];   /* number of signal bias */
    unsigned char smode[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT][RTCM_SSR_GP_MAX_SIG]; /* signal tracking mode */
    unsigned char fix[RTCM_SSR_MAX_GP][RTCM_SSR_GP_MAX_SAT][RTCM_SSR_GP_MAX_SIG];   /* fix status 1:fix */
    unsigned char update; /* update flag (0:no update,1:update) */
    int network;
} ssrgp_t;
...
typedef struct {        /* navigation data type */
    ...
    olod_t *oload[CSSR_MAX_NETWORK]; /* grid ocean loading */
    tec_t *tec;         /* tec grid data */
    stec_t *stec;       /* stec grid data */
    zwd_t *zwd;         /* zwd grid data */
    stec_t *stec_ch[SSR_CH_NUM]; /* stec grid data(2channel) */
    zwd_t *zwd_ch[SSR_CH_NUM];   /* zwd grid data((2channel)) */
    int ngp[CSSR_MAX_NETWORK]; /* number of gridded points */
    ...
    double nlbias[MAXSAT];     /* nide-lane bias (cycle) */
    double l1bias[MAXSAT];     /* L1 bias (cycle) */
    double l2bias[MAXSAT];     /* L2 bias (cycle) */
    gtime_t fcbtime;    /* fcb data time (GPST) */
    ...
    int updateac;       /* update flag (0:no update,1:update) */
    sft_t sfts;         /* 1/4cycle correction */
    int ni,nimax;       /* number of isb */
    isb_t *isb;         /* isb data */
    sta_t stas[MAXRCV]; /* station infomation */
    int facility[SSR_CH_NUM];
    int ionoreset;
    int filreset;
    int invtrop;        /* valid/invalid flag of trop correction (0:valid 1:invalid) */
} nav_t;
...
typedef struct {        /* solution type */
    ...
    double dop[4];      /* dop {GDOP,PDOP,HDOP,VDOP} */
    double chisq;       /* chi-sqr validaiton  */
    int network;
} sol_t;
...
typedef struct {        /* RTCM control struct type */
    ...
    int ctype;          /* current message type */
    int subtype;
    int mode;           /* meassge mode */
...
typedef struct {        /* processing options type */
    ...
    int qzsmodear;      /* QZSS AR mode (0:off,1:on,2:GPS-QZSS) */
    int maxout;         /* obs outage count to reset bias */
    int minlock;        /* min lock count to fix ambiguity */
    int minfix;         /* min fix count to hold ambiguity */
    int armaxdelsat;    /* max number of delete satellite for PAR */
    int ionoopt;        /* ionosphere option (IONOOPT_???) */
    int tropopt;        /* troposphere option (TROPOPT_???) */
    int dynamics;       /* dynamics model (0:none,1:velociy,2:accel) */
    int tidecorr;       /* earth tide correction (0:off,1:solid,2:solid+otl-station+pole,3:solid+otl-clasgrid+pole) */
    int niter;          /* number of filter iteration */
    int netnum;         /* network number (1~12) */
    int regularly;      /* filter reset cycle (s) */
    int epochtoretry;   /* unfix epoch count to reset filter */
    int retrycnt;       /* number of trials to reset filter */
    int gridsel;        /* 1 point grid select (m) */
    int floatcnt;       /* float counter for filter reset */
    int isbbyprn;       /* isb by prn (0:off,1:on) */
    int codesmooth;     /* code smoothing window size (0:none) */
    int intpref;        /* interpolate reference obs (for post mission) */
    int sbascorr;       /* SBAS correction options */
    int sbassatsel;     /* SBAS satellite selection (0:all) */
    int rovpos;         /* rover position for fixed mode */
    int alphaar;        /* AR validation threshold mode */
    int refpos;         /* base position for relative mode */
                        /* (0:pos in prcopt,  1:average of single pos, */
                        /*  2:read from file, 3:rinex header, 4:rtcm pos) */
    int minamb;         /* min number of ambiguities for PAR */
    int minhold;        /* min number of ambiguities for Hold */
    int estat;          /* extended statistics infomation (0:off,1:on) */
    double maxobsloss;  /* current and previous time difference threshold (s) */
    double forgetion;   /* forgetting factor of ionospheric delay for Adaptive Filter */
    double forgetpva;   /* forgetting factor of pos/vel/acc for Adaptive Filter */
    double afgainion;   /* gain of ionospheric delay for adaptive Filter */
    double afgainpva;   /* gain of pos/vel/acc for adaptive Filter */
    int    prnadpt;     /* adjust adaptively pos/vel/acc process noise */
    ...
    double prn[8];      /* process-noise std [0]bias,[1]iono [2]trop [3]acch [4]accv [5]posh [6]posv [7]ionomax */
    double sclkstab;    /* satellite clock stability (sec/sec) */
    double thresar[4];  /* AR validation threshold */
    double elmaskar;    /* elevation mask of AR for rising satellite (deg) */
    double elmaskhold;  /* elevation mask to hold ambiguity (deg) */
    double maxpdopar;   /* maximum pdop for AR */
    double maxpdophold; /* maximum pdop to hold ambiguity */
    int    refdop;      /* reference dop (0:conventional dop, 1: Sigle diff dop)*/
    double varholdamb;  /* constraint to hold ambiguity (cycle^2) */
    double thresslip;   /* slip threshold of geometry-free phase (m) */
    double rejethres;   /* threshold of pseudorange residuals rejection (m) */
    double rejeminsat;  /* mimimum satellites for pseudorange residuals rejection */
    double maxtdiff;    /* max difference of time (sec) */
    double maxageclk;   /* max age of clk correction (sec) */
    double maxageoth;   /* max age of ssr correction without clk (sec) */
    double maxinno[5];  /* reject threshold of innovation (sigma) */
    double maxdiffp;    /* reset state threshold of diffrencence between ppp-sol and pp-sol (m) */
    int    poserrcnt;   /* positioning error count to reset all states (cnt)*/
    ...
    double beta;        /* ionosphere time constant (Gauss-Markov) */
    int l6week;         /* L6 week */
    int l6dump;         /* L6 dump */
    int l6mrg;          /* L6 2ch mode */
} prcopt_t;
...
typedef struct {        /* solution options type */
    ...
    int osr;            /* osrfile output (0:off,1:on) */
...
typedef struct {        /* file options type */
    ...
    char grid   [MAXSTRPATH]; /* cssr grid file */
} filopt_t;
...
typedef struct {        /* satellite status type */
    ...
    double l0bias;      /* previous l0 phase bias correction (ssr) [m] */
    int pbreset[NFREQ]; /* phase bias reset flag */
    int outage;         /* out age flag */
    unsigned char code[NFREQ+NEXOBS]; /* code indicator (CODE_???) */
} ssat_t;
...
typedef struct {        /* observation data record */
    ...
    int iode;                     /* IODE */
    double clk;                   /* satellite time correction by SSR */
    double orb;                   /* satellite position correction by SSR */
    double trop;                  /* troposphere delay correction by SSR */
    double iono;                  /* ionosphere correction by SSR */
    double age;                   /* SSR age */
    double cbias[NFREQ+NEXOBS];   /* satellite code bias by SSR */
    double pbias[NFREQ+NEXOBS];   /* satellite phase bias by SSR */
    double l0bias;                /* l0 phase bias by SSR */
    double discontinuity[NFREQ+NEXOBS]; /* discontinuity indicator */
    double rho;                   /* distance between satellite and receiver */
    double dts;                   /* satellite clock correction from broadcast clock parameters */
    double relatv;                /* relativity delay (shapiro time delay) */
    double earthtide;             /* earthtide correction (solid earth tide and elimate permanent deformation) */
    ...
    double ddisp;                 /* SD disperse residual  (m) */
    double dL0;                   /* SD non-disperse residual (m) */
    double sis;
} osrd_t;
...
extern int  filter2(rtk_t *rtk, double *x, double *P, double *Q, const double *H, const double *v,
                   const double *R, const int n, const int m, const int *vflg, const int flg);  
extern int  filter_res(const double *x, const double *P, const double *H, const double *v,
                   const double *R, double *Q, int n, int m);
extern int  smoother(const double *xf, const double *Qf, const double *xb,
                     const double *Qb, int n, double *xs, double *Qs);
extern void matprint (const double *A, int n, int m, int p, int q);
extern void matfprint(const double *A, int n, int m, int p, int q, FILE *fp);
extern int dsvd(double *a, int n, int m, double *w, double *v);
...
extern int readblqgrid(const char *file, nav_t *nav);
...
extern double shapiro(const double *rsat, const double *rrcv);
...
extern void tidedisp(gtime_t tutc, const double *rr, int opt, const erp_t *erp,
                     const double *odisp, olod_t *oload, int netnum, int nn, 
                     int *index, double *Gmat, double *Emat, double *weight, double *dr);
...
extern double var_uraeph(int ura, int sys);
extern int ura_index(double value, int sys);
...
extern int satpos_ssr_sis(gtime_t time, gtime_t teph, int sat, rtk_t *rtk, nav_t *nav, int ch);
...
extern int lambda(int n, int m, const double *a, const double *Q, double *F,
                  double *s);
extern int parlambda(int n, int m, const double *a, const double *Q, const int alphaar,
                     double *F, double *s, int *nfix);
...
extern void update_global_cssr(ssr_t *ssr, int sat, int l6mrg);
extern void update_local_cssr(nav_t *nav, int l6mrg);
extern void check_cssr_facility(nav_t *nav, int network, int l6mrg);
extern gtime_t get_current_cssr_time(int idx);
extern gtime_t get_backup_cssr_time(int idx);
...
extern int is_sis_adjust(int idx);
extern void init_cssr_object(int file_idx);
...
/* grid */
extern int add_grid_idx(nav_t *nav, const double *pos, const int idx);
...
extern void clearsatcorr(void);
extern void getorbitclock(double tow, int sat, double *orbit, double *clock, int ch);
extern double adjust_cpc(gtime_t teph, int sat, const ssr_t *ssr, int sig, double cpc, double *sis, int *flag, int ch);
extern double adjust_prc(gtime_t teph, int sat, const ssr_t *ssr, int sig, double prc, double *sis, int *flag, int ch);
extern void adjust_r_dts(double *retr, double *retdts, gtime_t teph, int sat, nav_t *nav, double *rr, gtime_t dts_, int ch);
