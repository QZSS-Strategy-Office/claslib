...
static cssr_orbit_bank OrbitBank[ORBBANKNUM];
static cssr_clock_bank ClockBank[CLKBANKNUM];
static cssr_bias_bank  BiasBank[BIASBANKNUM];
static cssr_trop_bank  TropBank[TROPBANKNUM];
static int fastfix[CSSR_MAX_NETWORK] = {0};
static cssr_latest_trop LatestTrop;
static cssr_bank CurrentCSSR;
static cssr_bank BackupCSSR;
static grid_t BackupGrid;
static int Facility  = -1;
static int separation = 0;
...
static void check_cssr_changed_facility(int facility)
{
    if (Facility != facility) {
        ...
        memset(&LatestTrop, 0x00, sizeof(cssr_latest_trop));
...
static cssr_orbit_bank *get_same_orbit_correction(gtime_t time, int network)
{
    ...
    for (i = 0; i < ORBBANKNUM; ++i) {
        if (OrbitBank[i].use == TRUE && OrbitBank[i].network == network &&
            timediff(OrbitBank[i].time, time) == 0.0) {
            return &OrbitBank[i];
        }
    }
    return NULL;
...
{
    int search = ((separation & (1 << (network - 1))) ? network: 0);
    int pos = -1;
    int i;

    for (i = 0; i < ORBBANKNUM; ++i) {
        if (OrbitBank[i].use == TRUE) {
            trace(5, "get_close_orbit_correction(): orbit=%.1f, network=%d, diff=%.1f\n", time2gpst(OrbitBank[i].time, NULL),
                OrbitBank[i].network, timediff(OrbitBank[i].time, time));
        }
        if (OrbitBank[i].use == TRUE && OrbitBank[i].network == search && fabs(timediff(OrbitBank[i].time, time)) <= age) {
    ...
    if (pos != -1) {
        trace(4, "get_close_orbit_correction(): orbit=%.1f, network=%d, diff=%.1f\n", time2gpst(OrbitBank[pos].time, NULL),
            OrbitBank[pos].network, timediff(OrbitBank[pos].time, time));
...
void set_cssr_bank_orbit(gtime_t time, nav_t *nav, int network)
{
    ...
    if ((orbit = get_same_orbit_correction(time, network)) == NULL) {
        OrbitBank[NextOrbit].network = network;
    ...
    trace(4, "set_cssr_bank_orbit(): tow=%d, network=%d, next=%d\n",
        (int)time2gpst(time, NULL), network, NextOrbit);
}
...
static cssr_bias_bank *get_same_bias_correction(gtime_t time, int network)
{
    ...
    for (i = 0; i < BIASBANKNUM; ++i) {
        if (BiasBank[i].use == TRUE && BiasBank[i].network == network &&
            timediff(BiasBank[i].time, time) == 0.0) {
            return &BiasBank[i];
        }
    }
    return NULL;
    ...
    if (network >= 0 && network < CSSR_MAX_NETWORK) {
        for (i = 0; i < BIASBANKNUM; ++i) {
            if (BiasBank[i].use == TRUE) {
                trace(5, "get_close_cbias_correction(): bias=%.1f, network=%d, flag=0x%02x\n", time2gpst(BiasBank[i].time, NULL),
                    BiasBank[i].network, BiasBank[i].bflag);
            }
            if (BiasBank[i].use == TRUE && BiasBank[i].network == network && timediff(BiasBank[i].time, time) <= age) {
                if (pos != -1 && timediff(BiasBank[pos].time, BiasBank[i].time) > 0.0) {
                    continue;
                }
                if ((BiasBank[i].bflag & 0x01) == 0x01) {
                    pos = i;
                }
            }
        }
        if (pos != -1) {
            trace(4, "get_close_cbias_correction(): bias=%.1f, diff=%.1f, network=%d, flag=0x%02x\n",
                time2gpst(BiasBank[pos].time, NULL), timediff(BiasBank[pos].time, time),
                BiasBank[pos].network, BiasBank[pos].bflag);
            return &BiasBank[pos];
        }
    }
    return NULL;
...
void set_cssr_bank_cbias(gtime_t time, nav_t *nav, int network, int iod)
{
    ...
    if ((bias = get_same_bias_correction(time, network)) == NULL) {
        BiasBank[NextBias].network = network;
        BiasBank[NextBias].bflag = 0x00;
...
static cssr_clock_bank *get_same_clock_correction(gtime_t time, int network)
{
    ...
    for (i = 0; i < CLKBANKNUM; ++i) {
        if (ClockBank[i].use == TRUE && ClockBank[i].network == network &&
            timediff(ClockBank[i].time, time) == 0.0) {
            return &ClockBank[i];
        }
    }
    return NULL;
...
static cssr_clock_bank *get_close_clock_correction(gtime_t obstime, gtime_t time, int network, double age)
{
    ...
    for (i = 0; i < CLKBANKNUM; ++i) {
        if (ClockBank[i].use == TRUE) {
            trace(5, "get_close_clock_correction(): clock=%.1f, network=%d, diff=%.1f\n", time2gpst(ClockBank[i].time, NULL),
                ClockBank[i].network, timediff(ClockBank[i].time, time));
        }
        if (ClockBank[i].use == TRUE && ClockBank[i].network == search && timediff(ClockBank[i].time, time) < age) {
            if (pos != -1 && timediff(ClockBank[pos].time, ClockBank[i].time) > 0.0) {
                continue;
            }
            if (timediff(ClockBank[i].time, obstime) > 0.0) {
                continue;
            }
            pos = i;
        }
    ...
    if (pos != -1) {
        trace(4, "get_close_clock_correction(): clock=%.1f, network=%d, diff=%.1f\n", time2gpst(ClockBank[pos].time, NULL),
            ClockBank[pos].network, timediff(ClockBank[pos].time, time));
...
void set_cssr_bank_clock(gtime_t time, nav_t *nav, int network)
{
    ...
    if ((clock = get_same_clock_correction(time, network)) == NULL) {
        ClockBank[NextClock].network = network;
    ...
    trace(4, "set_cssr_bank_clock(): tow=%d, network=%d, next=%d\n",
        (int)time2gpst(time, NULL), network, NextClock);
}
...
static cssr_trop_bank *get_close_trop_correction(gtime_t obstime, gtime_t time, int network, double age, int tropless)
{
    ...
    trace(3, "get_close_trop_correction(): tow=%.1f, network=%d\n", time2gpst(time, NULL), network);
    ...
    if (network >= 1 && network < CSSR_MAX_NETWORK) {
        for (i = 0; i < TROPBANKNUM; ++i) {
            ...
            if (TropBank[i].use == TRUE && fabs(timediff(time, TropBank[i].time)) <= age) {
                ...
                if (tropless && timediff(obstime, TropBank[i].time) < 0.0) {
                    continue;
                }
...
void set_cssr_bank_trop(gtime_t time, ssrgp_t *ssrg, int network)
{
    ...
    trace(4, "set_cssr_bank_trop(): network=%d, gridnum=%d\n", network, trop->gridnum[network-1]);
    for (i = 0; i < ssrg->ngp; ++i) {
...
void set_grid_data(double *pos, int index)
{
    ...
    for (i = 0; i < CSSR_MAX_NETWORK - 1; ++i) {
        ...
        if (!orbit || !trop) {
            ...
            for (j = 0; j < RTCM_SSR_MAX_GP; ++j) {
                grid_stat[preliminary][j] = FALSE;
            }
            continue;
        }
        if (fastfix[network] == TRUE && timediff(time, trop->time) >= 0.0) {
            fastfix[network] = FALSE;
        }
        
        trace(4, "check_cssr_grid_status(): network=%2d, fastfix=%d, obs=%.1f, trop=%d\n",
            network, fastfix[network], tow, (int)time2gpst(trop->time, NULL));
        for (j = 0; j < trop->gridnum[i]; ++j) {
            if (trop->total[i][j] == INVALID_VALUE || trop->wet[i][j] == INVALID_VALUE) {
                trace(3, "%2d-%2d, total=%.4f, wet=%.4f\n", i, j, trop->total[i][j],
                    trop->wet[i][j]);
                grid_stat[network][j] = FALSE;
                continue;
            }
...
static int sub_get_close_cssr(gtime_t time, int network, cssr_orbit_bank **orbit, cssr_clock_bank **clock, cssr_bias_bank **cbias, cssr_bias_bank **pbias, cssr_trop_bank **trop, int *flag)
{
    ...
    if (!(*trop = get_close_trop_correction(time, (*orbit)->time, network, 30.0, (fastfix[network-1] ? FALSE: TRUE)))) {
...
extern int get_close_cssr(gtime_t time, int network)
{
    ...
    if (sub_get_close_cssr(time, network, &orbit, &clock, &cbias, &pbias, &trop, &sis) == FALSE) {
        return FALSE;
    }
    ...
    init_grid_index(&CurrentCSSR);
    init_grid_index(&saveCSSR);
        ...
        if (cbias->prn[i] != 0) {
            for (j = 0; j < MAXCODE; ++j) {
                if (cbias->smode[i][j] != 0 && (cbias->sflag[i][j] & 0x01) == 0x01) {
                    ...
                    CurrentCSSR.cbias[i][j] = cbias->cbias[i][j];
                    CurrentCSSR.smode[i][j] = cbias->smode[i][j];
                }
            }
        }
        if (pbias->prn[i] != 0) {
            for (j = 0; j < MAXCODE; ++j) {
                if (pbias->smode[i][j] != 0 && (pbias->sflag[i][j] & 0x02) == 0x02) {
                    ...
                    CurrentCSSR.pbias[i][j] = pbias->pbias[i][j];
                    CurrentCSSR.smode[i][j] = pbias->smode[i][j];
                }
            }
        }
        CurrentCSSR.time[i][4] = cbias->time;
        CurrentCSSR.prn[i][4] = cbias->prn[i];
        CurrentCSSR.udi[i][4] = cbias->udi[i];
        CurrentCSSR.iod[i][4] = cbias->iod[i];
        CurrentCSSR.time[i][5] = pbias->time;
        CurrentCSSR.prn[i][5] = pbias->prn[i];
    ...
    for (i = 0; i < MAXSAT; ++i) {
        if (orbit->prn[i] != 0) {
            ...
            CurrentCSSR.prn[i][0] = orbit->prn[i];
            CurrentCSSR.udi[i][0] = orbit->udi[i];
            CurrentCSSR.iod[i][0] = orbit->iod[i];
            CurrentCSSR.iode[i] = orbit->iode[i];
            CurrentCSSR.deph0[i] = orbit->deph0[i];
            CurrentCSSR.deph1[i] = orbit->deph1[i];
            CurrentCSSR.deph2[i] = orbit->deph2[i];
            CurrentCSSR.flag[i] |= 0x01;
        }
    }

    for (i = 0, CurrentCSSR.gridnum = trop->gridnum[network-1]; i < CurrentCSSR.gridnum; ++i) {
        for (j = 0, set_grid_data(&trop->gridpos[network-1][i][0], i); j < trop->satnum[network-1][i]; ++j) {
...
gtime_t get_current_cssr_time(void)
{
...
gtime_t get_backup_cssr_time(void)
{
...
{
    memcpy(&BackupCSSR, &CurrentCSSR, sizeof(cssr_bank));
    memcpy(&BackupGrid, grid, sizeof(grid_t));
    init_grid_index(&BackupCSSR);
}
...
extern void restore_current_cssr(gtime_t time, grid_t *grid)
{
    ...
    memcpy(&CurrentCSSR, &BackupCSSR, sizeof(cssr_bank));
    memcpy(grid, &BackupGrid, sizeof(grid_t));
    init_grid_index(&CurrentCSSR);
    
    if ((clock = get_close_clock_correction(time, CurrentCSSR.orbit_time, grid->network, 30.0))) {
        if (timediff(CurrentCSSR.clock_time, clock->time) != 0.0) {
            trace(2, "restore_current_cssr(): network=%2d, diff=%.1f, orbit=%d, clock=%d\n",
                grid->network, timediff(clock->time, CurrentCSSR.orbit_time),
                (int)time2gpst(CurrentCSSR.orbit_time, NULL),
...
extern void clear_current_cssr(void)
{
    ...
    init_grid_index(&BackupCSSR);
    memset(&CurrentCSSR, 0x00, sizeof(cssr_bank));
    init_grid_index(&CurrentCSSR);
}
...
{
    int i, j, k, s, sync, tow, iod, ngnss, sat[CSSR_MAX_SV], nsat, sz_idx, sz_idx_t, sz_idx_s[CSSR_MAX_SV], sz;
    int trop_avail, stec_avail, trop_type=-1, stec_type=-1, inet, a, b, quality, trop_qual=-1, stec_qual[CSSR_MAX_SV], stec_type_[CSSR_MAX_SV];
    double total[CSSR_MAX_GP], wet[CSSR_MAX_GP], stec[CSSR_MAX_GP][CSSR_MAX_SV];
    double udint, stec0, ct[6]={0}, ci[6]={0}, ci_[CSSR_MAX_SV][6]={0}, trop_ofst, trop_residual, dlat, dlon, dstec;
    nav_t *nav = &rtcm->nav;
    ssrgp_t *ssrg;
