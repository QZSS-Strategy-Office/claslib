...
#define MAX_AGE     29.0       /* max age of difference (s) */
                     ...
                     double *rate, double *rms, double *quality, int *slip)
{
    ...
    if (flag == 0 || stec->data[k].flag == -1) {
        trace(3,"search_data: no iono (no sat data) %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    
    tt=timediff(time,stec->data[k].time);
    if (fabs(tt)>MAXAGESSR_IONO) {
        trace(2,"age of ssr iono error %s sat=%2d tt=%.0f\n",time_str(time,0),sat,tt);
        stec->data[k].flag = -1;
        return 0;
    }
                    ...
                    int *brk)
{
    ...
    double ionos[MAX_NGRID],rates[MAX_NGRID],rms[MAX_NGRID],quality[MAX_NGRID],sum;
    int i,slip;
