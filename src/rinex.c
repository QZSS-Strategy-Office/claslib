...
static int addsftdata(sft_inf *sft, const sftd_t *data)
{
    sftd_t *sft_data;

    if (sft->nmax<=sft->n) {
        if (sft->nmax<=0) sft->nmax=NINCOBS; else sft->nmax*=2;
        if (!(sft_data=(sftd_t *)realloc(sft->data,sizeof(sftd_t)*sft->nmax))) {
            trace(1,"addsftdata: memalloc error n=%dx%d\n",sizeof(sftd_t),sft->nmax);
            free(sft->data); sft->data=NULL; sft->n=sft->nmax=0;
            return -1;
        }
        sft->data=sft_data;
    }
                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    else if (strstr(label,"SYS / PHASE SHIFT"   )) { /* ver.2.12 */
        if (sta) {
            if (!(p=strchr("G",buff[0])) && !(p=strchr("J",buff[0]))) {
                trace(2,"invalid system code: sys=%c\n",buff[0]);
                return;
            }

            if (buff[0]=='G') sys =SYS_GPS;
            else if (buff[0]=='J') sys =SYS_QZS;

            data.sys= sys;

            if (ver<=2.99){
                setstr(code,buff+2,2);
                convcode(ver,sys,code,str);
                data.code=obs2code(str+1,NULL);
            } else {
                setstr(code,buff+2,3);
                data.code=obs2code(code+1,NULL);
            }

            data.phasecorr = str2num(buff,6,8);
            n=(int)str2num(buff,16,2);
            data.n=n;
            for (i=nt=0,j=18;i<n;i++,j+=4) {
                if (j>58) {
                    if (!fgets(buff,MAXRNXLEN,fp)) break;
                    j=18;
                }
                if ((sys== SYS_GPS&& nt>=MAXPRNGPS-1) || (sys== SYS_QZS && nt>=NSATQZS-1)) continue;
                setstr(id,buff+j+1,3);

                data.sats[i] = satid2no(id);

                nt++;
            }
            if(n > 0){
                addsftdata(&sta->sft,&data);
            }
        }
    }
}
                      ...
                      eph_t *eph)
{
    ...
    *eph=eph0;
    ...
    if (sys==SYS_GPS||sys==SYS_QZS) {
        ...
        eph->sva=ura_index(data[23],sys);  /* ura (m->index) */
                       ...
                       seph_t *seph)
{
    ...
    *seph=seph0;
    ...
    seph->svh=(int)data[6];
    seph->sva=ura_index(data[10],satsys(sat,NULL));
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
    ...
    outnavf(fp,sqrt(var_uraeph(eph->sva,sys)));
    outnavf(fp,eph->svh    );
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph)
{
    ...
    outnavf(fp,sqrt(var_uraeph(seph->sva,sys)));
    fprintf(fp,"\n%s",sep         );
