...
static int test_sys(int sys, int m, int qzsmodear)
{
    switch (sys) {
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    for (m=0;m<5;m++) /* m=0:gps/qzs/sbs,1:glo,2:gal,3:bds,4:qzs */
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (i=-1,j=0;j<ns;j++) {
            ...
            if (!test_sys(sysi,m,opt->qzsmodear)) continue;
        ...
        for (j=0;j<ns;j++) {
            ...
            if (!test_sys(sysj,m,opt->qzsmodear)) continue;
            if (!validobs(iu[j],ir[j],f,nf,y)) continue;
    ...
    for (m=0;m<5;m++) { /* m=0:gps/qzs/sbs,1:glo,2:gal,3:bds,4:qzs */
        ...
        for (f=0,k=na;f<nf;f++,k+=MAXSAT) {
            ...
            for (i=k;i<k+MAXSAT;i++) {
                if (rtk->x[i]==0.0||
                    !test_sys(rtk->ssat[i-k].sys,m,rtk->opt.qzsmodear)||
                    !rtk->ssat[i-k].vsat[f]) {
                    continue;
                }
            ...
            for (j=k;j<k+MAXSAT;j++) {
                if (i==j||rtk->x[j]==0.0||
                    !test_sys(rtk->ssat[j-k].sys,m,rtk->opt.qzsmodear)||
                    !rtk->ssat[j-k].vsat[f]) {
                    continue;
                }
...
static void restamb(rtk_t *rtk, const double *bias, int nb, double *xa)
{
    ...
    for (m=0;m<5;m++) for (f=0;f<nf;f++) {
        ...
        for (n=i=0;i<MAXSAT;i++) {
            if (!test_sys(rtk->ssat[i].sys,m,rtk->opt.qzsmodear)||
                          rtk->ssat[i].fix[f]!=2) {
                continue;
            }
...
#ifndef CSSR2OSR_VRS
void output_osr_txt( FILE *fp, const obsd_t *obs, const osrd_t *osr, const int n, double *pos)
#else
...
#endif
{
    ...
    for (i=0;i<n;i++){
         ...
         sys=satsys(osr[i].sat,&prn);
         if (osr[i].p[0]==0.0||osr[i].c[0]==0.0) continue;
         k = i;
#else
         tow = time2gpst(obs->data[i].time, NULL);
         sys = satsys(obs->data[i].sat, &prn);
         /* osr->data[].sat */
         k = obs->data[i].osr_i;
#endif
         if (fp) {
              fprintf(fp, "OSRRES,%.1f,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,"
                 "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,"
                 "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.9f,%.9f,%.3f\n",
                 tow, sys, prn,
                 osr[k].pbias[0], osr[k].pbias[1], osr[k].pbias[2],
                 osr[k].cbias[0], osr[k].cbias[1], osr[k].cbias[2],
                 osr[k].trop,
                 osr[k].iono,
                 osr[k].antr[0], osr[k].antr[1], osr[k].antr[2],
                 osr[k].relatv,
                 osr[k].wupL[0], osr[k].wupL[1], osr[k].wupL[2],
                 osr[k].compL[0], osr[k].compL[1], osr[k].compL[2],
                 osr[k].sis,
                 osr[k].CPC[0], osr[k].CPC[1], osr[k].CPC[2],
                 osr[k].PRC[0], osr[k].PRC[1], osr[k].PRC[2],
                 osr[k].orb, osr[k].clk,
                 pos[0]*R2D,pos[1]*R2D,pos[2]);
         }
    }
                          ...
                          const double *x,  const int n, prcopt_t *opt)
{
    ...
    if (opt->tidecorr) {
        tidedisp(gpst2utc(obs[0].time),rr,1+2+4+8,&nav->erp,opt->odisp[0],
                 NULL,0,0,NULL,NULL,NULL,NULL,disp);
        for (i=0;i<3;i++) rr[i]+=disp[i];
    }
    ecef2pos(rr,pos);
    output_osr_txt( fp_osr, obs, osr, n, pos);
...
#endif
{
    ...
    if (!pntpos(obs,nu,nav,&rtk->opt,&rtk->sol,NULL,rtk->ssat,msg)) {
        ...
        if (!rtk->opt.dynamics && opt->mode<PMODE_PPP_RTK) {
            outsolstat(rtk);
            return 0;
        }
    }
...
#endif
#ifdef ENA_SSR2OSR
    if (opt->mode>=PMODE_SSR2OSR) {
        ssr2osr(rtk,obs,nu,nav,osr,0);
        write_osr(obs,nav,osr,rtk->sol.rr,nu,&rtk->opt);
        outsolstat(rtk);
        return 1;
    }
#endif
    ...
    if (opt->mode>=PMODE_PPP_KINEMA&&opt->mode!=PMODE_VRS_RTK) {
